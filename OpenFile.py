...
from mount import  addr_space, part_info, inode_table,data, current_inode, get_dir_table, read_dict, update_file_link_n_parent_dir,free_blocks,out_scope_block
from error import free_space_not_found,out_of_scope_memory_accessed_in_file,trimming_empty_file,operation_not_for_folders
...
class OpenFile:
    ...
    def read(self,start=0, end=-1):
        global inode_table;
        if inode_table[self.inode][9][0]==-1:
            return "";
        ...
        # get addr_list
        address_list = inode_table[self.inode][9]
        #
        # print(end_block_no)
        # print(start_block_no)
        if end_block_no - start_block_no == 0:
            # print(data[address_list[start_block_no]][displacement4start:end + 1])
            return data[address_list[start_block_no]][displacement4start:end + 1]
        first_chunk = data[address_list[start_block_no]][displacement4start:]
        last_chunk = data[address_list[end_block_no]][:displacement4end + 1]
        ...
        for i in range(len(address_list)):
            mid_chunk = mid_chunk + data[address_list[i]]
    ...
    def write(self,text, loc=0):
        ...
        if inode_table[self.inode][9][0] == -1:
            ...
            for i in range(required_blocks):
                new_addrs = free_blocks.pop(0)
                blocks.extend([new_addrs])
                if new_addrs == out_scope_block:
            ...
            inode_table[self.inode][9] = blocks
            ...
            for i in range(len(blocks)):
                if len(text) - block_size > 0:
                    ...
                    data[blocks[i]] = text[0:block_size]
                    text = text[block_size:]
                else:
                    if len(data) == blocks[i]:
                        data.append(text)
                    else:
                        data[blocks[i]] = text
        else:
            ...
            block_addr_list = inode_table[self.inode][9]
            ...
            while (len(text) != 0 and len(block_addr_list) != 0):
                ...
                if loc == 0 or loc % block_size == 0:
                    if len(text) - block_size > 0:
                        data[addr] = text[0:block_size]
                        text = text[block_size:]
                    else:
                        copied = data[addr][len(text):0]
                        data[addr] = text + copied
                        return
                else:
                    copied = data[addr][0:loc]
                    ...
                    if index2:
                        ...
                        end_block = data[addr][len(start_block):]
            ...
            # update addr_list for this inode
            inode_table[self.inode][9].extend(blocks)
    ...
    def truncate(self, size):
        ...
        if size == 0:
            return
        addr_list = inode_table[self.inode][9]
        if addr_list[0] == -1:
            trimming_empty_file()
            return
        ...
        if size < block_size:
            addr = addr_list[-1]
            data_len = len(data[addr]);
            if size >= data_len:
                inode_table[self.inode][9].pop(-1);
                if len(inode_table[self.inode][9]) == 0:
                    inode_table[self.inode][9] = [-1]
                data[addr] = "deleted";
                size = size - data_len
            else:
                new_data = data[addr][0:data_len - size];
                data[addr] = new_data;
            ...
            addr = inode_table[self.inode][9].pop(-1);
            if len(inode_table[self.inode][9]) == 0:
                inode_table[self.inode][9] = [-1]
            data[addr] = "deleted";
...
def Open(fname):
    ...
    if inode_table[file_inode][1] == 0:
        operation_not_for_folders()
        return

    return OpenFile(fname=fname, mode="this para is no longer needed here", parent_inode=current_inode, file_inode=file_inode)
